...
parser.add_argument('--epoch', type=int, default=220, help='epoch number')
...
image_root = '/data/datasets/train_data-augment/train_images/train_images/'
gt_root = '/data/datasets/train_data-augment/train_masks/train_masks/'
depth_root = '/data/datasets/train_data-augment/train_depth/train_depth/'
edge_root = '/data/datasets/train_data-augment/train_edge/'
val_root = '/data/datasets/'
validation = ['NLPR']
...
def train_calibration(calibration_loader, model_rgb,model_depth, model_discriminator, model_estimator,
    ...
    for i , pack in enumerate(tqdm(calibration_loader),start=1):
        ...
        atts_depth, pred_depth, _, _, _ = model_estimator(images)
        loss_dep = (smo_L1(atts_depth, depths) + smo_L1(pred_depth, depths)) /2.0
...
def val(dataset_path, test_datasets, ckpt_name):
    ...
    for dataset in test_datasets:
        ...
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        image_ro = dataset_path + dataset + '/test_data/Image/'
        gt_ro = dataset_path + dataset + '/test_data/Mask/'
        depth_ro = dataset_path + dataset + '/test_data/Depth/'
...
def load_two_stream_RANKING(key_min):
    ...
    for i in trange(test1_loader.size):
        ...
        if len(os.listdir(pos_path)) <= 0.30*len(test1_loader):
            if iou_d >= iou_r:
        ...
        if i+1 == test1_loader.size:
            ...
            if len(os.listdir(pos_path)) <= 0.30*len(test1_loader):
                ...
                for item_i in sort_iou:
                    if item_i[0] in curr_pos_list:
                        pass
                    elif len(os.listdir(pos_path)) <= 0.30 * len(test1_loader):
            ...
            if len(os.listdir(neg_path)) <= 0.30 * len(test1_loader):
