...
parser.add_argument('--snapshot', type=int, default=45, help='load checkpoint number')
...
def train(train_loader, model_rgb, model_depth, model,
    ...
    for i, pack in enumerate(tqdm(train_loader), start=1):
        ...
        with torch.no_grad():
            ...
            score1 = -torch.log(score)
            score2 = -torch.log(1 - score)
            _,pred_depth,_,_,_ = model_estimator(images)
            depth_calibrated = torch.mul(depths,score2[:,0].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).
                                         expand(-1, 1, opt.trainsize, opt.trainsize)) \
                               + torch.mul(pred_depth,score1[:,1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).
                                         expand(-1, 1, opt.trainsize, opt.trainsize))
